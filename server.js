...
// Mappa: clientName → PushSubscription
...
async function sendPush(targetName, payload) {
  ...
  try {
    ...
    if (err.statusCode === 410 || err.statusCode === 404) {
      pushSubscriptions.delete(targetName);
    }
  }
}
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (data) => {
    try {
      ...
      // Push ai telefoni offline quando arriva un msg dal PC
      if (clientType === 'pc') {
      ...
      // Push al PC non si fa (è un client Python), ma si potrebbe aggiungere
      // un'email/webhook in futuro
